...
//   },[courseSectionData, courseEntireData, location.pathname])
  useEffect(() => {
    const setVideoSpecificDetails = async () => {
      if (!courseId || !sectionId || !subSectionId) {
        console.error("Incomplete IDs provided");
        return;
      }
  
      if (!courseSectionData || courseSectionData.length === 0) {
        console.error("No course section data available");
        return;
      }
      ...
      const filteredSubSection = filteredSection.subSection.find((data) => data._id === subSectionId);
      if (!filteredSubSection) {
        console.error("Sub-section not found");
        return;
      }
